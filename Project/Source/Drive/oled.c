...
void OLED_ShowFloat(u8 x,u8 y,float num,u8 len, u8 size2)
{
	...
	temp_int_num = (int)num;
	temp_deci_num = (num - temp_int_num) * pow(10, len);
	
	OLED_ShowNum(x,y,temp_int_num, 2, size2);
	OLED_ShowChar(x+2*6, y,'.');
	OLED_ShowNum(x+(2+1)*6,y,temp_deci_num, len, size2);
}
